...
- precision:
  ...
  (integer) desired number of decimal places for coordinates. Using
  fewer decimal places decreases object sizes (at the cost of
  precision). This changes the underlying precision stored in the data.
  `options(digits = <some number>)` changes the maximum number of digits
  displayed (to find out what yours is set at see
...
- ...:
  ...
  For sf classes, data.frames, lists, numerics, and geo_lists, it is
  passed through to
...
This function creates a geojson structure as a json character string; it
does not write a file - see
...
objects, while other classes (numeric, list, data.frame) can be output
as `FeatureCollection` or `GeometryCollection` objects. We're working on
allowing `GeometryCollection` option for sp class objects.

Also note that with sp classes we do make a round-trip, using
[`sf::st_write()`](https://r-spatial.github.io/sf/reference/st_write.html)
to write GeoJSON to disk, then read it back in. This is fast and we
don't have to think about it too much, but this disk round-trip is not
ideal.
...
- sfc: `GeometryCollection`, unless the sfc is length 1, then the
  geometry as above
...
The `digits` parameter of
[`jsonlite::toJSON`](https://jeroen.r-universe.dev/jsonlite/reference/fromJSON.html)
controls precision for classes `numeric`, `list`, `data.frame`, and
`geo_list`.

For `sp` classes, precision is controlled by
[`sf::st_write`](https://r-spatial.github.io/sf/reference/st_write.html),
being passed down through
[`geojson_write()`](https://docs.ropensci.org/geojsonio/reference/geojson_write.md),
then through internal function `write_geojson()`, then another internal
function `write_ogr_sf()`
